...
class Trace_ELBO(ELBO):
    ...
    def _differentiable_loss_particle(self, model_trace, guide_trace, elbo_lambdas):
        elbo_particle = 0
        ...
        for name, site in model_trace.nodes.items():
            ...
            if site["type"] == "sample":
                # print(name + " P loss")
                elbo_particle = elbo_particle + torch_item(site["log_prob_sum"]) * self.elbo_lambdas[idx]
                surrogate_elbo_particle = surrogate_elbo_particle + site["log_prob_sum"] * self.elbo_lambdas[idx]
                idx = idx + 1
